    ...
    <section id="vouchers" className="py-[30px] md:py-20 w-full bg-black overflow-hidden">
      ...
      >
        {vouchers.map((voucher, idx) => (
          <div key={idx} className="relative w-[280px] h-[110px] sm:w-[440px] sm:h-[160px] md:w-[576px] md:h-[208px] shrink-0 select-none">
            {/* Scalloped cutouts */}
            ...
            >
              ...
              <div className="w-[90px] sm:w-[150px] md:w-[200px] bg-white flex items-center justify-center shrink-0 rounded-l-[10px] sm:rounded-l-[14px] overflow-hidden">
                <img
                  src="/nike-logo.jpg"
                  alt="Nike"
                  className="max-h-[40px] sm:max-h-[65px] md:max-h-[85px] max-w-[80%] object-contain sm:scale-[1.4] sm:translate-x-[6px] sm:-translate-y-[6px] md:scale-[1.5] md:translate-x-[10px] md:-translate-y-[8px]"
                />
              </div>
              ...
              {/* Right: Info section */}
              <div className="flex-1 flex flex-col justify-between p-3 sm:p-4 md:p-5 min-w-0 rounded-r-[10px] sm:rounded-r-[14px] overflow-hidden">
                {/* Top: discount + brand */}
                <div>
                  {/* Mobile: separate lines | sm+: combined */}
                  <h4 className="text-[16px] sm:text-[24px] md:text-[33px] font-medium text-white leading-tight mb-0.5 sm:mb-2">
                    <span className="sm:hidden">{voucher.discount}</span>
                    <span className="hidden sm:inline">{voucher.discount} | {voucher.brand}</span>
                  </h4>
                  {/* Brand — mobile only */}
                  <p className="sm:hidden text-[12px] font-semibold text-white leading-none mb-1">{voucher.brand}</p>
                  ...
                  {/* Expiry */}
                  <p className="text-[9px] sm:text-[10.5px] md:text-[12px] text-white/30 font-normal leading-none mt-1 sm:mt-3">
                    {voucher.expiry}
                  </p>
                </div>
                {/* Flags — hidden on mobile */}
                <div className="hidden sm:flex justify-start items-center border-t border-white/5 pt-2 md:pt-2.5">
